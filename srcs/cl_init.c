...
{
	frc->cl_structs.int_mem = clCreateBuffer(frc->cl_structs.context, CL_MEM_READ_WRITE,
							sizeof(int) * 32, NULL, &frc->cl_structs.ret);
	frc->cl_structs.mem_img = clCreateBuffer(frc->cl_structs.context, CL_MEM_READ_ONLY,
							sizeof(int) * WIDTH * HEIGHT, NULL, &frc->cl_structs.ret);
	frc->cl_structs.double_mem = clCreateBuffer(frc->cl_structs.context, CL_MEM_READ_WRITE,
								sizeof(double) * 32, NULL, &frc->cl_structs.ret);
	frc->cl_structs.palette = clCreateBuffer(frc->cl_structs.context, CL_MEM_READ_WRITE,
								sizeof(int) * (frc->iter + 1), NULL, &frc->cl_structs.ret);
...
void				init_cl(t_fractol_base *frc)
{
	...
	frc->cl_structs.program = clCreateProgramWithSource(frc->cl_structs.context, 1,
	...
	if(frc->cl_structs.ret < 0)
	{
		clGetProgramBuildInfo(frc->cl_structs.program,
			frc->cl_structs.device, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
		program_log = ft_strnew(log_size);
		program_log[log_size] = '\0';
		clGetProgramBuildInfo(frc->cl_structs.program,
						frc->cl_structs.device, CL_PROGRAM_BUILD_LOG,
			log_size + 1, program_log, NULL);
		printf("%d\n%s\n", frc->cl_structs.ret, program_log);
		ft_strdel(&program_log);
		exit(1);
	}
...
void			run_cl(t_fractol_base *frc)
{
	...
	ret = clEnqueueWriteBuffer(frc->cl_structs.queue, frc->cl_structs.double_mem,
	...
	ret = clEnqueueNDRangeKernel(frc->cl_structs.queue, frc->cl_structs.kernel,
		1, NULL, &kernel_num, NULL, 0, NULL, NULL);
	ret = clEnqueueReadBuffer(frc->cl_structs.queue, frc->cl_structs.mem_img, CL_TRUE,
			0, sizeof(int) * frc->width * frc->height, frc->img->ptr_data, 0, NULL, NULL);
	mlx_put_image_to_window(frc->ptr_mlx, frc->ptr_window, frc->img->ptr_img, 0, 0);
}
